...
const Connect = () => {
  ...
  -d '{
    ...
    "scopes": ["read", "write"]
          ...
          <StepCard number={4} icon={FileCode} title="Wire up the Opix client">
            <p>
              Drop a tiny client file into your project so the rest of your code can{" "}
              <code className="text-foreground font-mono">{`import { opix } from "@/integrations/opix/client"`}</code>{" "}
              instead of repeating fetch calls everywhere.
            </p>
            <p className="text-sm">
              Create <code className="text-primary font-mono">src/integrations/opix/client.ts</code>:
            </p>
            <CodeBlock
              lang="typescript"
              code={`// This file is automatically generated. Do not edit it directly.
  ...
  return {
    ...
    listEvents: (params?: { limit?: number; since?: string }) => {
      ...
      return request(\`/events-list\${qs ? \`?\${qs}\` : ""}\`);
          ...
          <StepCard number={5} icon={Send} title="Track your first event">
